...
void pimontecarlo(size_t threadID, size_t numThreads, size_t N, double &pi) {
  ...
  // use a local 'cache' for the pi value
  double local_pi = 0;
  ...
  // lock around writing to the pi value
  unique_lock<mutex> lk(pi_mutex);
  pi += local_pi;
}
...
int main(int argc, char *argv[]) {
  ...
  size_t N = 100000000;
  cout << "Generating " << N << " random values using " << numThreads
       << " thread(s)." << endl;
